@AbapCatalog.sqlViewName: '/EY1/PROTATBCGC'
@AbapCatalog.compiler.compareFilter: true
@AbapCatalog.preserveKey: true
@AccessControl.authorizationCheck: #CHECK
@EndUserText.label: 'I-View-PR-OTA Taxable Inc/Loss Before Compensation Losses GC'
@VDM.viewType: #COMPOSITE

define view /EY1/SAV_I_PR_OTA_TILBCL_GC
  with parameters
    p_toperiod      : poper,
    p_ryear         : gjahr,
    p_taxintention : zz1_taxintention
  as select from    /EY1/SAV_I_PR_S2T_FPLBT_GC
                 ( p_toperiod:$parameters.p_toperiod,
                       p_ryear:$parameters.p_ryear,
                       p_taxintention:$parameters.p_taxintention)      as TaxableProfitLossBeforeTaxS2T

    left outer join /EY1/SAV_I_PR_PTA_Total_GC
                    ( p_toperiod:$parameters.p_toperiod,
                        p_ryear:$parameters.p_ryear,
    ...
    left outer join /EY1/SAV_I_PR_OTA_Total_GC
                    ( p_toperiod:$parameters.p_toperiod,
                            p_ryear:$parameters.p_ryear,
  ...
  association [0..1] to /EY1/SAV_C_CurrLocalGroupVH as _Cntry on  TaxableProfitLossBeforeTaxS2T.ConsolidationUnit = _Cntry.ConsolidationUnit
                                                              and TaxableProfitLossBeforeTaxS2T.GroupCurrency     = _Cntry.Currency
{
  ...
  key TotalPTA.FiscalYear,
      ...
      cast (case when FiscalIncomeLossBT is null then cast( case
                                                     when TotalPTA is null then TotalOTA
                                                     when TotalOTA is null then TotalPTA
                                                     else TotalPTA + TotalOTA end as abap.curr(23,2))
           when TotalPTA is null then cast(case
                                                       when FiscalIncomeLossBT is null then TotalOTA
                                                       when TotalOTA is null then FiscalIncomeLossBT
                                                       else TotalOTA + FiscalIncomeLossBT end as abap.curr(23,2))
           when TotalOTA is null then cast(case
                                              when FiscalIncomeLossBT is null then TotalPTA
