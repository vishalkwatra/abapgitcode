@AbapCatalog.sqlViewName: '/EY1/ERS2TIRC'
@AbapCatalog.compiler.compareFilter: true
@AbapCatalog.preserveKey: true
@AccessControl.authorizationCheck: #CHECK
@EndUserText.label: 'I-View ER S2T Rec Check'
...
define view /EY1/SAV_I_ER_S2T_RecChk
  ...
  as select from    /EY1/SAV_I_ER_TaxGaapEquiTot(p_ryear:$parameters.p_ryear,
    ...
    left outer join /EY1/SAV_I_ER_S2T_RechChkLCGC( p_ryear:$parameters.p_ryear ,
                                                                                                  ...
                                                                                                  and RecChk.CurrencyType                 = GLAcc.CurrencyType
{
  ...
  key GLAcc.FiscalYear,
      ...
      case
      when OBBalance is null then cast( 0 as abap.curr(23,2)) - TAXOpeningBalance
      when TAXOpeningBalance is null then OBBalance
      else OBBalance - TAXOpeningBalance
      end as ReconciliationCheckOB,
      @Semantics.amount.currencyCode: 'MainCurrency'
      case
      when YBBalance is null then cast( 0 as abap.curr(23,2)) - YBClosingBalance
      when YBClosingBalance is null then YBBalance
      else YBBalance - YBClosingBalance
      end as ReconciliationCheckYB,
      @Semantics.amount.currencyCode: 'MainCurrency'
      case
      when CBBalance is null then cast( 0 as abap.curr(23,2)) - CBClosingBalance
      when CBClosingBalance is null then CBBalance
      else CBBalance - CBClosingBalance
      end as ReconciliationCheckCB
